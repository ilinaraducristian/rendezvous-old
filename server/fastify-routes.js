...
function fastifyRoutes(fastify, io) {
    ...
    async function createServer(req, res) {
        try {
            const [newRow] = await ServerModel.save(req.body.name, req.auth.sub);
            await MemberModel.save(req.auth.sub, newRow.insertId);
            res.send({id: newRow.insertId});
        } catch (err) {
            console.log(err);
            res.code(500).send();
        }
    ...
    async function createInvitation(req, res) {
        ...
        try {
            const [[member]] = await MemberModel.findByUserIdAndServerId(req.auth.sub, serverId);
            if (member === undefined) {
                res.code(400);
                return {error: 'You are not a member of this server'};
            }
            let [[invitation]] = await InvitationModel.findByServerId(serverId);
            ...
            [invitation] = await InvitationModel.save(serverId, invitationCode, new Date(new Date().getTime() + 5 * 60000));
            return {invitation: invitationCode, id: invitation};
        } catch (err) {
            console.log(err);
            res.code(500).send();
        }
    ...
    async function joinServer(req, res) {
        ...
        try {
            const [[invitation]] = await InvitationModel.findByInvitation(invitationCode);
            if (invitation === undefined) {
                res.code(400);
    ...
    async function createChannel(req, res) {
        ...
        const [newChannel] = await ChannelModel.save(server.id, req.body['channel_name']);
        return {id: newChannel.insertId};
    }
    ...
    function parseServerData(serversData) {
        ...
        serversData.forEach(row => {
            ...
            if (channel_id > lastChannelId) {
                ...
                servers[servers.length - 1].channels.push([channel_id, channel]);
            }
            ...
            servers[servers.length - 1].channels[servers[servers.length - 1].channels.length - 1].messages.push([message_id, message]);
        });
        return servers;
    }
