...
class DBTest(unittest.TestCase):
    ...
    def test_server_creation(self):
        try:
            cursor.callproc(
                "create_server", [self.uid1, "new server"]
            )
            cursor.callproc(
                "get_user_servers_data", [self.uid1]
            )
            db.commit()
            db.close()
    ...
    def test_group_creation(self):
        try:
            ...
            cursor.execute(
                f"SELECT create_group('{self.uid1}', {server_id}, 'new group')"
            )
        ...
        try:
            ...
            for group in cursor.fetchall():
                ...
                if group[2] == 'Voice channels':
                    gid2 = group[0]
            cursor.execute(
                f"SELECT create_channel('{self.uid1}', {server_id}, {gid1}, 'text', 'new text channel')"
            )
            text_channel = cursor.fetchall()
            cursor.execute(
                f"SELECT create_channel('{self.uid1}', {server_id}, {gid2}, 'voice', 'new voice channel')"
            )
    ...
    def test_invitation_creation(self):
        try:
            ...
            server_id = cursor.stored_results().__next__().fetchall()[0][0]
            cursor.execute(
                f"SELECT create_invitation('{self.uid1}', {server_id})"
            )
    ...
    def test_join_server(self):
        try:
            ...
            invitation = cursor.fetchall()
            cursor.callproc(
                "join_server", [self.uid2, invitation[0][0]]
            )
